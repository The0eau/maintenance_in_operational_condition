...
### La Plateforme Rotative (Châssis et Indexation)
...
### Le Système de Positionnement (Capteurs)
...
# On utilise le plot par défaut de flexsurv qui superpose les données et Weibull
plot(fit_flex, 
...
# Ajout d'une légende pour la clarté
legend("topright", 
...
```{r}
# --- Optimisation des Mèches de Perçage ---
...
3.  Calcul du taux de défaillance par choc ($\lambda_{choc}$) : On calcule d'abord la probabilité moyenne de rupture par choc $P_{choc}$ en pondérant les risques de chaque état d'usure de la mèche par sa distribution stationnaire $\pi$ : $$P_{choc} = \sum_{i=1}^{n} P(choc | État_i) \cdot \pi_i$$

    Le taux de défaillance par choc est alors défini par : $$\lambda_{choc} = - \ln(1 - P_{choc})$$

À l'instant $t = 500h$, la probabilité de survie globale du moteur est donc : $$R_{moteur}(500) = \exp\left(-\left(\frac{500}{\eta_{mot}}\right)^{\beta_{mot}}\right) \cdot \exp(-\lambda_{choc} \cdot 500)$$
...
# calculée dans la boucle d'optimisation pour l'indice 3 (Préventif_E4)
indispo_meche_tete <- prob_maint  # Issue de du code "Optimisation Mèches" pour E4
...
# Utilise MTTF_global calculé dans le point moteur
