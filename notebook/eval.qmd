...
index_opti <- which.min(couts_horaires) # Trouve l'indice du coût le plus bas
optimum_etat <- names(index_opti)

# CALCUL DE LA FRÉQUENCE OPTIMALE RÉELLE
# On refait le calcul pour l'état sélectionné (ou on le récupère si on l'avait stocké)
if(index_opti <= 3) {
  # Si c'est du préventif (E2, E3 ou E4)
  etat_cible <- index_opti + 1
  frequence_opti <- pi_glob[etat_cible] * abs(Q_glob[etat_cible, etat_cible])
} else {
  # Si c'est du correctif (E5)
  frequence_opti <- pi_glob[5] * abs(Q_glob[5,5])
}

cat("LA MEILLEURE STRATÉGIE EST :", optimum_etat, "\n")
cat("Fréquence optimale :", round(frequence_opti, 6), "remplacements/heure\n")
cat("Soit environ :", round(frequence_opti * 8760, 1), "fois par an\n")
cat("Cycle moyen (T_M) :", round(1/frequence_opti, 2), "heures\n")
...
nb_interv_meche_an <- frequence_opti * 8760
...
L'analyse de la maintenance opportuniste révèle une disparité critique entre les cycles de vie des composants, l'optimum de lubrification étant désormais fixé à une fréquence très basse de $k^ = 0,5$ intervention par an\* (soit environ une fois tous les deux ans). Bien que le remplacement des mèches soit fréquent (53 fois/an) et que le gain par intervention groupée soit de 28 minutes (705 € d'économie de non-production), la rareté de l'opération de lubrification bride structurellement le potentiel de synergie : à l'échelle d'une station, l'économie de 352,55 € est négligeable face au budget d'exploitation global.
...
dur_L <- 4.0  ; cout_L <- 900   # Lubrification
dur_P <- 3.0  ; cout_P <- 1100   # Plateforme

# FRÉQUENCES OPTIMALES
T_M <- 1 / frequence_opti            # Cycle Mèches (53 fois/an)
T_C <- tau_opti            # Cycle Capteurs (26 fois/an)
freq_L_an <- k_star      # Lubrification (1 fois tous les 2 ans)
T_P <- T_opti          # Âge optimal Plateforme (T*)
...
L'analyse démontre que la stratégie des Poupées Russes est la seule viable économiquement, permettant une économie nette de 69 378 € par an pour l'atelier par rapport à une maintenance isolée. En synchronisant le remplacement des capteurs sur les arrêts fréquents des mèches et en groupant les opérations lourdes (lubrification et plateforme) lors d'un grand entretien bisannuel, on réduit drastiquement les pertes de production liées aux temps d'arrêt (1500 €/h). À l'inverse, l'alignement forcé de tous les composants sur le cycle le plus court (le "Grand Arrêt" toutes les 165h) s'avère catastrophique, générant une perte de plus de 2,2 millions d'euros due au remplacement prématuré et injustifié de pièces coûteuses encore fonctionnelles. Cette approche opportuniste transforme ainsi la maintenance d'un centre de coûts subi en un levier d'optimisation de la marge opérationnelle de l'usine.