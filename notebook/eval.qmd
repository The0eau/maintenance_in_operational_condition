...
nb_stations <- 10
cout_heure_arret <- 1500  
duree_lub_seule   <- 4.0   
duree_meche_seule <- 0.67  
duree_intervention_groupee <- 4.2  

# 3. Calcul des gains
# Gain de temps par groupage (en heures)
gain_temps_par_regroupement <- (duree_lub_seule + duree_meche_seule) - duree_intervention_groupee
# Gain financier par opération
gain_argent_par_regroupement <- gain_temps_par_regroupement * cout_heure_arret

# On ne peut grouper que selon l'événement le moins fréquent (la lubrification)
nb_regroupements_par_station <- min(nb_interv_lub_an, nb_interv_meche_an)

# Économie pour TOUT L'ATELIER (10 stations)
economie_annuelle_atelier <- nb_regroupements_par_station * gain_argent_par_regroupement * nb_stations
...
cat("====================================================\n")
cat("Besoins annuels par station :\n")
cat(" -> Maintenance Lubrification :", round(nb_interv_lub_an, 2), "fois/an\n")
...
cat("Nombre de groupages possibles :", round(nb_regroupements_par_station, 2), "fois/an\n")
cat("----------------------------------------------------\n")
cat("ÉCONOMIE ANNUELLE ATELIER (10 STATIONS) :", round(economie_annuelle_atelier, 2), "€/an\n")
cat("====================================================\n")
}
```
...
# 1. Paramètres de l'arrêt groupé (Plateforme + Lubrification)
cout_heure_indispo_station <- 1500 
duree_plateforme           <- 3.0
duree_lub_complete         <- 5.0
duree_intervention_groupee  <- 5.5

# 2. Définition des fréquences (Utilisation de t_star_plateforme du Point 2)
# On aligne la lubrification sur le cycle de la plateforme
freq_plateforme_opti <- 8760 / t_star_plateforme  
freq_lub_annuel_opti <- freq_plateforme_opti      

# 3. Calcul du gain financier du groupage
# Nombre de groupages possibles par an et par station
nb_groupages_an_station <- freq_plateforme_opti 

# Gain de temps unitaire : (3h + 5h) - 5.5h = 2.5h
gain_temps_unitaire  <- (duree_plateforme + duree_lub_complete) - duree_intervention_groupee
gain_argent_unitaire <- gain_temps_unitaire * cout_heure_indispo_station

# 4. Impact global sur l'atelier (10 stations)
nb_stations <- 10
economie_totale_atelier <- gain_argent_unitaire * nb_groupages_an_station * nb_stations

# 5. Affichage des résultats
cat("====================================================\n")
cat("   RÉ-OPTIMISATION : STRATÉGIE DE GROUPAGE TOTAL    \n")
cat("====================================================\n")
cat("Nouvelles fréquences alignées (Atelier 10 stations) :\n")
cat(" -> Fréquence Plateforme ré-optimisée :", round(freq_plateforme_opti, 2), "fois/an\n")
cat(" -> Fréquence Lubrification alignée   :", round(freq_lub_annuel_opti, 2), "fois/an\n")
cat("----------------------------------------------------\n")
cat("Bilan du gain (Optimisation Systémique) :\n")
cat(" -> Heures gagnées par station/an     :", round(gain_temps_unitaire * nb_groupages_an_station, 2), "h\n")
cat(" -> Gain financier par station/an     :", round(gain_argent_unitaire * nb_groupages_an_station, 2), "€\n")
cat("----------------------------------------------------\n")
cat("ÉCONOMIE TOTALE ATELIER (10 STATIONS) :", round(economie_totale_atelier, 2), "€/an\n")
cat("====================================================\n")


# --- PRÉPARATION DES DONNÉES ---
# Scénario 1 : Interventions séparées (3h + 5h)
# Scénario 2 : Intervention groupée (5.5h)
valeurs <- c(duree_plateforme + duree_lub_complete, duree_intervention_groupee)
noms <- c("Séparées (Cumul)", "Groupée (Optimisée)")

# --- CONFIGURATION DU GRAPHIQUE ---
# On augmente la marge à gauche pour les labels
par(mar=c(5, 10, 4, 2)) 
...
bp <- barplot(valeurs, 
              horiz = TRUE, 
              names.arg = noms, 
              col = c("#E74C3C", "#2ECC71"), # Rouge pour perte, Vert pour gain
              border = "white",
              main = "Optimisation du Temps d'Arrêt (Point 5)",
              xlab = "Heures d'arrêt machine",
              xlim = c(0, 10),
              las = 1)

# Ajout des étiquettes de temps sur les barres
text(x = valeurs - 0.5, y = bp, labels = paste(valeurs, "h"), 
     col = "white", font = 2, cex = 1.2)

# --- VISUALISATION DU GAIN ---
# Dessiner une flèche montrant l'économie de temps
arrows(x0 = 8, y0 = bp[1], x1 = 5.5, y1 = bp[2], 
       col = "darkgreen", lwd = 2, length = 0.1)

# Ajouter le texte du gain financier
legend("topright", 
       legend = c(paste("Gain de temps :", gain_temps_unitaire, "h"),
                  paste("Économie / station :", round(gain_argent_unitaire, 0), "€"),
                  paste("TOTAL ATELIER :", round(economie_totale_atelier, 0), "€/an")),
       bty = "n", cex = 0.9, text.col = "darkgreen", text.font = 2)

# Réinitialiser les marges
par(mar=c(5, 4, 4, 2))
```

L'analyse des résultats démontre de manière univoque la pertinence de la nouvelle stratégie de maintenance groupée, qui s'aligne sur les réalités économiques du système. En délaissant le couplage entre les mèches et la lubrification au profit d'une synchronisation des interventions lourdes (Plateforme et Nettoyage complet), vous avez identifié un levier d'optimisation majeur : le gain de 2,5 heures de temps machine par arrêt groupé permet de limiter l'indisponibilité totale de la station, dont le coût est critique (1 500 €/h). Ce pivot stratégique transforme une économie annuelle anecdotique de 352 € en un gain substantiel de 37 500 € pour l'ensemble des 10 stations, prouvant ainsi que la rentabilité ne réside pas dans la fréquence des regroupements, mais dans la mutualisation des composants dont la maintenance paralyse l'intégralité de la chaîne de production.